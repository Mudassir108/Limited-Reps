...
function detectProductType(category, name) {
  // Check for accessories in name FIRST (highest priority to avoid conflicts)
  if (name.includes('bag') || name.includes('cap') || name.includes('hat') ||
      ...
      name.includes('maison') || name.includes('margiela') || name.includes('replica') || name.includes('bottle') ||
