...
function detectProductType(category, name) {
  ...
  if (name.includes('tape') || name.includes('adhesive') || name.includes('duct tape') ||
      ...
      name.includes('decal') || name.includes('decals')) {
    return 'tape'
  }
  
  // Check for bags THIRD (should show clothing sizes and colors)
  if (name.includes('bag') || name.includes('duffel') || name.includes('duffle') ||
